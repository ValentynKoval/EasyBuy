...
public final class ShopSpecifications {
    ...
    public static Specification<Shop> likeNameOrDescription(String keyword) {
        return (root, query, cb) -> {
            ...
            // We convert the search query to lowercase and add % masks.
            String pattern = "%" + keyword.toLowerCase() + "%";

            // Create predicates for fields defined in the Shop entity [1]
            Predicate nameLike = cb.like(cb.lower(root.get("shopName")), pattern);
            Predicate descriptionLike = cb.like(cb.lower(root.get("shopDescription")), pattern);

            // We combine conditions using logical "OR"
